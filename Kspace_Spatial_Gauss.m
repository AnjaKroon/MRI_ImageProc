...
% name of sliceX_channelX_MODNAME for the three files of interest (can maybe not have to pass in)
% OUTPUTS
% finalized eye picture
...
Data_img(:,:,1) = ifftshift(ifft2(slice2_channel1_gauss),1);
%channel 2
Data_img(:,:,2) = ifftshift(ifft2(slice2_channel2_gauss),1);
%channel 3
Data_img(:,:,3) = ifftshift(ifft2(slice2_channel2_gauss),1);
...
%figure(2); 
%imagesc(100*log(abs(slice2_channel1_gauss)));
%xlabel('Horizontal frequency bins')
%ylabel('Vertical frequency bins');

%figure(3); 
%imagesc(100*log(abs(slice2_channel2_gauss)));
%xlabel('Horizontal frequency bins')
%ylabel('Vertical frequency bins');

%figure(4); 
%imagesc(100*log(abs(slice2_channel3_gauss)));
%xlabel('Horizontal frequency bins')
%ylabel('Vertical frequency bins');
