...
% images generated for good data and for a secondary comparison metric
% here, the comparison metric is pred
...
load('slice2_channel1_pred.mat');
load('slice2_channel2_pred.mat');
load('slice2_channel3_pred.mat');
whos
...
butter(:,:,1) = ifftshift(ifft2(slice2_channel1_pred),1);
%channel 2
butter(:,:,2) = ifftshift(ifft2(slice2_channel2_pred),1);
%channel 3
butter(:,:,3) = ifftshift(ifft2(slice2_channel3_pred),1);


%%%%%%% CLEAR COMP %%%%%%%
% clear compensation, preparation, based on fourier transforpred blinked 
